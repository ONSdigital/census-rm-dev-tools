...
pipenv run python -m whitelist_service_ip $WFH_IP ops || exit 1
pipenv run python -m whitelist_service_ip $WFH_IP rabbitmqmanagement || exit 1
pipenv run python -m whitelist_service_ip $WFH_IP case-api-test || exit 1
pipenv run python -m whitelist_db_ip $WFH_IP "$WFH_NAME" census-rm-whitelodge
...
pipenv run python -m whitelist_service_ip $WFH_IP rabbitmqmanagement || exit 1
pipenv run python -m whitelist_service_ip $WFH_IP case-api-test || exit 1
pipenv run python -m whitelist_db_ip $WFH_IP "$WFH_NAME" census-rm-blacklodge
